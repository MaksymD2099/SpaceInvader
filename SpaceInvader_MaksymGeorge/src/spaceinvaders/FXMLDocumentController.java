    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        new AnimationTimer() {
            ...
            public void handle(long now) {
                ...
                try {
                    if (getRightMostEnemy().getPosition().getX() >= 750) {
                        for (int i = 0; i < 4; i++) {
                            for (int j = 0; j < 8; j++) {
                                if (enemies[i][j] != null) {
                                    enemies[i][j].setPosition(new Vector2D(enemies[i][j].getPosition().getX(), enemies[i][j].getPosition().getY() + 10)); //-------------------------------
                ...
                //Check if the Enemy Passed the Safe line and killed the Spaceship
                for (int i = 0; i < 4; i++) {
                    for (int j = 0; j < 8; j++) {
                        
                        if( enemies[i][j] != null && enemies[i][j].getRectangle().getY() >= 500)
                        {
                              try {
                                    ...
                                    for (int k = 0; k < objectList.size(); k++) {
                                        if (objectList.get(k) != null) {
                                            pane.getChildren().remove(objectList.get(k).getRectangle());
                                        }
                                    }
